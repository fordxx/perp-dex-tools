    ...
    def get_account_balances(self) -> List[Balance]:
        ...
        for raw in balances_data or []:
            try:
                ...
                logger.warning("Failed to parse balance payload on %s: %s (error: %s)", self.name, raw, type(exc).__name__)
            except Exception as exc:
                logger.exception("Unexpected error parsing balance payload on %s: %s", self.name, exc)
        return balances
                    ...
                    async with websockets.connect(self.ws_url, ping_interval=15) as ws:
                        ...
                        async for msg in ws:
                            try:
                                ...
                                logger.warning("Invalid JSON in %s order stream: %s", self.name, msg[:100])
                            except Exception as exc:
                                logger.exception("Error handling %s order stream message: %s", self.name, exc)
                except (ConnectionError, OSError) as exc:
                    logger.warning("%s WebSocket connection error: %s (will retry)", self.name, exc)
                    await asyncio.sleep(5)
                except Exception as exc:  # pragma: no cover - network dependent
                    logger.exception("Unexpected error in %s order stream: %s", self.name, exc)
                    await asyncio.sleep(5)
