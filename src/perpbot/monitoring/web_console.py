...
class TradingService:
    ...
    def set_min_profit_pct(self, value: float) -> None:
        ...
        if value > 1.0:
            raise ValueError("Minimum profit threshold must not exceed 100%")
        with self._lock:
    ...
    def run_cycle(self) -> None:
        with self._lock:
            ...
            if self.state.quotes:
                try:
                    first_quote = next(iter(self.state.quotes.values()))
                    min_profit_abs = self.cfg.arbitrage_trade_size * first_quote.mid
