...
def single_cycle(cfg: BotConfig, state: TradingState) -> None:
    ...
    positions = []
    exchange_health = {}  # Track exchange health
    for ex in exchanges:
        try:
            ...
            exchange_health[ex.name] = "healthy"
        except (ConnectionError, TimeoutError) as exc:
            logger.warning("Exchange %s connection error: %s", ex.name, exc)
            exchange_health[ex.name] = "unhealthy"
        except NotImplementedError:
            # 某些交易所不支持该查询
            logger.debug("Exchange %s does not support position queries", ex.name)
            exchange_health[ex.name] = "unsupported"
        except Exception as exc:
            logger.exception("Failed to retrieve positions from %s: %s", ex.name, exc)
            exchange_health[ex.name] = "error"

    # Log exchange health status
    unhealthy_count = sum(1 for status in exchange_health.values() if status in ("unhealthy", "error"))
    if unhealthy_count > 0:
        logger.warning("Warning: %d/%d exchanges are unhealthy", unhealthy_count, len(exchanges))
    ...
    if state.quotes:
        try:
            first_quote = next(iter(state.quotes.values()))
            min_profit_abs = cfg.arbitrage_trade_size * first_quote.mid
    ...
    for quote in state.quotes.values():
        ...
        # Find exchange safely with default None check
        matching_exchange = next((ex for ex in exchanges if ex.name == quote.exchange), None)
        if matching_exchange:
            strategy.maybe_trade(state, matching_exchange, spread_signal, quote, cfg.position_size)
