...
# Precision tolerance for floating-point comparisons in capital management
CAPITAL_PRECISION_TOLERANCE = 1e-8
...
class PoolState:
    ...
    def allocate(self, amount: float) -> bool:
        """Allocate capital from pool. Uses precision tolerance for floating-point comparisons."""
        if amount < 0:
            logger.warning("Cannot allocate negative amount: %.8f", amount)
            return False

        if amount <= self.available + CAPITAL_PRECISION_TOLERANCE:
            self.allocated += amount
            return True

        logger.warning(
            "Allocation failed for pool %s: requested %.8f, available %.8f",
            self.name, amount, self.available
        )
        return False

    def release(self, amount: float) -> None:
        """Release allocated capital. Handles floating-point precision carefully."""
        if amount < 0:
            logger.warning("Cannot release negative amount: %.8f", amount)
            return

        old_allocated = self.allocated
        self.allocated = max(self.allocated - amount, 0.0)

        if abs((old_allocated - amount) - self.allocated) > CAPITAL_PRECISION_TOLERANCE:
            logger.debug("Release: pool=%s, released=%.8f, allocated changed from %.8f to %.8f",
                        self.name, amount, old_allocated, self.allocated)

    def validate(self) -> bool:
        """Validate pool state integrity. Returns True if valid."""
        if self.pool < 0 or self.allocated < 0:
            logger.error("Invalid pool state: pool=%.8f, allocated=%.8f", self.pool, self.allocated)
            return False
        if self.allocated > self.pool + CAPITAL_PRECISION_TOLERANCE:
            logger.error("Allocated exceeds pool: allocated=%.8f, pool=%.8f (excess=%.8f)",
                        self.allocated, self.pool, self.allocated - self.pool)
            return False
        return True
