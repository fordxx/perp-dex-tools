...
class ArbitrageExecutor:
    ...
    def execute(
        ...
        if self.risk_manager:
            ...
            for ex in (buy_ex, sell_ex):
                try:
                    ...
                    logger.warning("网络错误导致获取余额失败 [%s]: %s", ex.name, exc)
                except Exception as exc:
                    logger.debug("获取余额失败 [%s]: %s", ex.name, type(exc).__name__)
        ...
        try:
            ...
            # Expected error during partial fill hedge
            logger.warning("套利执行被中断: %s", exc)
            ...
            logger.warning("网络错误导致套利执行失败: %s", exc)
            self._hedge_incomplete_legs(opportunity, buy_ex, sell_ex, buy_order, sell_order)
            self.guard.mark_failure()
            if self.risk_manager:
                self.risk_manager.record_failure()
            ...
            if self.recorder:
                self.recorder.record_trade(opportunity, success=False, actual_profit=0.0, error_message=str(exc))
            return ExecutionResult(opportunity, status="failed", error=f"Network error: {exc}")
